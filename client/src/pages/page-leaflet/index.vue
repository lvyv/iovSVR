  ...
  <d2-container>
    <div style="height: 40%; overflow: auto;">
      ...
      <table>
        ...
        <tr v-for="(item, index) in markers" :key="item.id">
      ...
      <table>
        ...
        <tr v-for="(item, index) in stuff" :key="item.id">
          <td>{{"Layer " + (index + 1)}}</td>
          <td style="text-align: center"><input v-model="item.visible" type="checkbox"></td>
          ...
          <td><button name="button" @click="fitPolyline">Fit map to polyline</button></td>
        </tr>
      </table>
      <hr/>
    </div>
    <l-map style="height: 300px;" :zoom.sync="zoom" :options="mapOptions"
      ...
      <l-tile-layer v-for="tileProvider in tileProviders"
          ...
          :url="tileProvider.url" :attribution="tileProvider.attribution" :token="token" :key="tileProvider.id"/>
    ...
    <div>
      ...
      <select v-model="layersPosition">
        <option v-for="position in Positions" :value="position" :key="position.index">{{position}}</option>
      </select>
      ...
      level: <input v-model="zoom" type="number"><br/>
...
export default {
  ...
  data () {
    return {
      ...
      markers: [
        {id: 'm1', position: {lat: 51.505, lng: -0.09}, tooltip: 'tooltip for marker1', draggable: true, visible: true, icon: L.icon.glyph({prefix: '', glyph: 'A'})},
      ...
      polylines: [
        {id: 'p1', points: [[37.772, -122.214], [21.291, -157.821], [-18.142, -181.569], [-27.467, -206.973]], visible: true},
